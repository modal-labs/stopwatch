...
class Benchmark(Base):
    ...
    vllm_docker_tag = Column(
        String, default=BenchmarkDefaults.VLLM_DOCKER_TAG, nullable=False
    )
    vllm_env_vars = Column(
        JSON, default=BenchmarkDefaults.VLLM_ENV_VARS, nullable=False
    )
    vllm_extra_args = Column(
        JSON, default=BenchmarkDefaults.VLLM_EXTRA_ARGS, nullable=False
    )
    ...
    def get_config(self):
        return {
            ...
            "gpu": self.gpu,
            "region": self.region,
            "vllm_docker_tag": self.vllm_docker_tag,
            "vllm_env_vars": self.vllm_env_vars,
            "vllm_extra_args": self.vllm_extra_args,
        }

    def save_results(self, results, vllm_metrics):
        if not results:
        ...
        for key in [
            ...
            setattr(
                ...
                np.median(
                    ...
                    [getattr(b, key) for b in benchmarks if getattr(b, key) is not None]
                ),
            )
